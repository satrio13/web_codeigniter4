...
class RekapUS extends BaseController
{
    ...
    public function hapus_rekap_us($id)
    {
        ...
        if($cek)
        {
            $this->m_us->hapus_rekap_us($id);
            if($this->db->affectedRows() > 0)
            {
                return redirect()->to(base_url('backend/rekap-us'))->with('success', 'Data Berhasil Dihapus');
            }else
            {
                return redirect()->to(base_url('backend/rekap-us'))->with('error', 'Data Gagal Dihapus, silahkan coba lagi!');
            }
        }else
        {
            show_404();
